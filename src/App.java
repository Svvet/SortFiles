...
public class App {
    static boolean toFolder(File fl) {
        ...
        try {
            ...
            if (type.equals("xml")) {
                Path copied = Paths.get("Foldery/DEV/" + name);
                ...
                if (hour % 2 == 0) {
                    ...
                    Path copied = Paths.get("Foldery/TEST/" + name);
                    Path originalPath = fl.toPath();
                    Files.copy(originalPath, copied, StandardCopyOption.REPLACE_EXISTING);
                    fl.delete();
                    return true;
    ...
    public static void main(String[] args) {
        int counter = 0;
        try {
            File myObj = new File("Foldery/HOME/count.txt");
            if (myObj.createNewFile()) {
                System.out.println("count.txt created: " + myObj.getName());
            } else {
                System.out.println("count.txt already exists.");
            }
            FileWriter myWriter = new FileWriter("Foldery/HOME/count.txt", false);
        ...
        try {
            ...
            Path path = Paths.get("Foldery/HOME");
            ...
            while ((key = watchService.take()) != null) {
                for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    File fll = new File("Foldery/HOME/" + filename);
